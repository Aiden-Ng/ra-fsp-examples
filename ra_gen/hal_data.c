...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.9999786666666667 seconds. Actual duty: 49.997866621154586%. */.period_counts = (uint32_t) 0x5b8d,
  .duty_cycle_counts = 0x2dc6, .source_div = (timer_source_div_t) 10, .channel = 1, .p_callback = timer0_callback,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .add_average_count = ADC_ADD_OFF,
  ...
  .adc_vref_control = ADC_VREF_CONTROL_2_5V_OUTPUT,
  .enable_adbuf = 0,
...
const adc_window_cfg_t g_adc0_window_cfg =
{
    ...
    .compare_ref_low     = 10000,
    .compare_ref_high    = 24824,
...
{ .scan_mask = ADC_MASK_CHANNEL_1 | 0, .scan_mask_group_b = 0, .priority_group_a = ADC_GROUP_A_PRIORITY_OFF, .add_mask =
          0,
